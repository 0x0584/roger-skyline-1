...
*** Setup acces with public keys

#+BEGIN_QUOTE
_NOTE:_

The process of creating publib/private key pair is *done on the Mac OS X machine*. Where we're going to send the public key to teh server to know who were are when interacting.
#+END_QUOTE
...
Next step is to copy the =id_rsa.pub= to the server using =ssh-copy-id= (WHAT! THEY HAVE COMMAND FOR THAT TOO!) as =ssh-copy-id -i id_rsa.pub <USER>@<IP> -p <PORT>=. In my case, USER is =archid-= IP is =10.11.200.247= and PORT is =1412=.
